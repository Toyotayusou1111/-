...
const raw = Object.fromEntries(
  emptyAreas.map((k) => [k, MAX_TOTAL * (ratios[k] / 1)])
);
...
const scaleTotal = MAX_TOTAL / Object.values(raw).reduce((a, b) => a + b, 0);
const finalScale = Math.min(1, scale2, scale3, scaleTotal);
...
👉 <strong>{emptyAreas.join(", ")}</strong> が未入力です </div>
)}
{emptyAreas.length > 0 && (
\<div style={{ marginTop: "1rem" }}> <strong>各エリア別 積載目安（第2軸10t & 第3軸20t & 合計19700kg内）</strong> <ul>
{Object.entries(recommended).map((\[k, v]) => ( <li key={k}>
{k}：{v.toLocaleString()}kg </li>
))} </ul> </div>
)} </div>
);
}
