...
recommended\[area] = Math.round(
(remainingAxle \* ratio) / influences\[area]
);
});

```
const scale = remainingTotal /
  Object.values(recommended).reduce((acc, val) => acc + val, 0);
...
{(usedLoad || 0).toLocaleString()}kg </div> <div> <strong>あと積める目安：</strong>
{(MAX\_AXLE\_LOAD - usedLoad).toLocaleString()}kg </div>
{emptyAreas.length > 0 && (
