...
const MAX\_AXLE2\_LOAD = 10000;
const MAX\_AXLE3\_LOAD = 20000;
...
const remainingAxle2 = Math.max(0, MAX\_AXLE2\_LOAD - usedAxle2);
const remainingAxle3 = Math.max(0, MAX\_AXLE3\_LOAD - usedAxle3);
...
const recommended = {};
if (emptyAreas.length > 0 && remainingAxle2 > 0 && remainingAxle3 > 0 && remainingTotal > 0) {
const ratios = {
mid1: 0.211,
mid2: 0.323,
rear: 0.279,
...
const rawAxle3 = Object.entries(rawRecommended).reduce(
  (acc, [key, val]) => acc + (key === "mid2" || key === "rear" ? val * influences[key] : 0),
  0
);

const scale2 = (MAX_AXLE2_LOAD - frontAxle) / (rawAxle2 - frontAxle);
const scale3 = MAX_AXLE3_LOAD / rawAxle3;
const scale = Math.min(scale2, scale3);
...
{Math.round(remainingAxle3).toLocaleString()}kg（第3軸） </div>
{emptyAreas.length > 0 && (
...
{emptyAreas.length > 0 && (
\<div style={{ marginTop: "1rem" }}> <strong>各エリア別 積載目安（第2軸10t & 第3軸20t & 合計19700kg）</strong> <ul>
