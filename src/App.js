...
export default function App() {
  ...
  if (emptyAreas.length > 0 && remainingAxle > 0 && remainingTotal > 0) {
    const sumSquare = emptyAreas.reduce(
      (acc, area) => acc + Math.pow(influences[area], 2),
      0
    );
    let rawRecommended = {};
    emptyAreas.forEach((area) => {
      const ratio = Math.pow(influences[area], 2) / sumSquare;
      rawRecommended[area] = (remainingAxle * ratio) / influences[area];
    });

    const totalRaw = Object.values(rawRecommended).reduce(
      (acc, val) => acc + val,
      0
    );
