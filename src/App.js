...
export default function App() {
  ...
  const getAreaWeight = (a) =>
    data[a].reduce((sum, val, i) => {
      const n = parseFloat(val); return isNaN(n) ? sum
        : sum + n * INITIAL_ROWS[a].coeffs[i];
    }, 0);

  const getTotalWeight = () =>
    data.reduce((sum, _, a) => sum + getAreaWeight(a), 0);
